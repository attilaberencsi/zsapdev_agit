...
CLASS LHC_SHOP IMPLEMENTATION.
  ...
  READ ENTITIES OF ZR_ShopTP_B IN LOCAL MODE
    ENTITY Shop
  ...
  SELECT MAX( ORDER_ID ) FROM ZSAPDEV_ASHOP INTO @DATA(max_object_id).
  "Add support for draft if used in modify
  "SELECT SINGLE FROM FROM ZSHOP00D_B FIELDS MAX( OrderID ) INTO @DATA(max_orderid_draft). "draft table
  "if max_orderid_draft > max_object_id
  " max_object_id = max_orderid_draft.
  "ENDIF.
  MODIFY ENTITIES OF ZR_ShopTP_B IN LOCAL MODE
    ENTITY Shop
